...
func (f *frameRateCalculatorVPx) calc() bool {
	...
	for currentTemporal := int32(0); currentTemporal <= DefaultMaxLayerTemporal; currentTemporal++ {
		...
		// 已经计算了较低的时间层，但较高的层没有接收到任何帧，它不应该存在
	...
	if rateCounter == len(f.frameRates) {
		...
		// 标准化帧速率，Microsoft Edge为vp8使用了3个临时层，但中间层有机会
		// 得到一个非常低的帧速率，所以我们需要标准化帧速率（对该层使用最高层的固定比率1:2）
...
// FrameRateCalculatorVP8 VP8中基于PictureID的帧速率计算器
...
// FrameRateCalculatorVP9 VP9中基于PictureID的帧速率计算器
type FrameRateCalculatorVP9 struct {
	logger    logger.Logger
	completed bool

	// VP9-TODO-假设有三个空间层。由于“已完成”标记依赖于正在完成的所有层，因此必须假设这一点。修复。
	// 也许可以看看tc中的层数。测量广告层后，TrackInfo并声明已完成
	frameRateCalculatorVPx [DefaultMaxLayerSpatial + 1]*frameRateCalculatorVPx
}
...
func NewFrameRateCalculatorVP9(clockRate uint32, logger logger.Logger) *FrameRateCalculatorVP9 {
	...
	for i := range f.frameRateCalculatorVPx {
		f.frameRateCalculatorVPx[i] = newFrameRateCalculatorVPx(clockRate, logger)
	}
...
func (f *FrameRateCalculatorVP9) RecvPacket(ep *ExtPacket) bool {
	...
	if ep.Spatial < 0 || ep.Spatial >= int32(len(f.frameRateCalculatorVPx)) || f.frameRateCalculatorVPx[ep.Spatial] == nil {
		f.logger.Debugw("invalid spatial layer", "sn", ep.Packet.SequenceNumber, "spatial", ep.Spatial)
		return false
	}

	success := f.frameRateCalculatorVPx[ep.Spatial].RecvPacket(ep, vp9.PictureID)

	completed := true
	for _, frc := range f.frameRateCalculatorVPx {
		if !frc.Completed() {
			completed = false
			break
		}
	...
	if completed {
		...
		for i := range f.frameRateCalculatorVPx {
			frameRates[i] = f.frameRateCalculatorVPx[i].GetFrameRate()
		}
...
func (f *FrameRateCalculatorVP9) GetFrameRateForSpatial(spatial int32) []float32 {
	if spatial < 0 || spatial >= int32(len(f.frameRateCalculatorVPx)) || f.frameRateCalculatorVPx[spatial] == nil {
		return nil
	}
	return f.frameRateCalculatorVPx[spatial].GetFrameRate()
}
...
// FrameRateCalculatorDD 基于依赖描述符的FrameRateCalculator
