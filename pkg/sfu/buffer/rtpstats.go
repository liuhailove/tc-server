...
import (
	...
	"github.com/liuhailove/tc-base-go/mediatransportutil"
)
...
// RTCPSenderReportData RTCP发送者报告数据
...
func (r *RTPStats) Seed(from *RTPStats) {
	...
	// 不要克隆 endTime，因为非零 endTime 表示对象已结束
	...
	r.packetsDuplicate = from.packetsPadding
...
func (r *RTPStats) Update(rtph *rtp.Header, payloadSize int, paddingSize int, packetTime time.Time) (flowState RTPFlowState) {
	...
	default:
		...
		if rtph.Timestamp != r.highestTS {
			...
			// 仅更新第一个数据包，因为多个数据包中可能存在相同的时间戳。
			// 注意：如果存在数据包丢失，这可能不是带有此时间戳的第一个数据包。
			r.highestTime = packetTime
...
func (r *RTPStats) UpdateFromReceiverReport(rr rtcp.ReceptionReport) (rtt uint32, isRttChanged bool) {
	...
	if !r.initialized || !r.endTime.IsZero() || !r.params.IsReceiverReportDriven || rr.LastSequenceNumber < r.extStartSN {
		// 当使用虚拟数据包触发 Pion 的 OnTrack 路径时，
		// 接收器报告中的“LastSequenceNumber”可能位于起始序列号之前。
		return
	}
...
func (r *RTPStats) maybeAdjustFirstPacketTime(ts uint32) {
	...
	// 开始后一段时间，调整第一个数据包的时间。
	// 有助于提高预期时间戳计算的准确性。
	// 仅调整一种方式，例如如果第一个样本经历了异常延迟（可能是由于节奏或可能是由于排队）
	// 在沿途的某个网络元素中，将第一次推回到较早的实例。
...
func (r *RTPStats) SetRtcpSenderReportData(srData *RTCPSenderReportData) {
	...
	if r.srNewest != nil {
		if srDataCopy.RTPTimestampExt < r.srNewest.RTPTimestampExt {
			// 当轨道被替换为 null 然后恢复时，可能会发生这种情况 -
			// i.e.静音替换为 null 并取消静音恢复原始轨道。
			// 在这种情况下，重置发送者报告以从此时开始。
			// 重置将确保采样率计算不会因负时间而失控。
			r.logger.Infow(
...
func (r *RTPStats) GetRtcpSenderReport(ssrc uint32, calculatedClockRate uint32) *rtcp.SenderReport {
	...
	// 发布者的节奏可能较慢。
	// 这将使 `highestTS` 落后于发布者的 RTCP 发送方报告中的 RTP 时间戳。
	// 使用计算的时钟速率检查这一点，并使用稍后的时间戳（如果适用）。
	tcCycles := r.tsCycles
	if nowRTP < r.highestTS {
		tcCycles++
	}
	nowRTPExt := getExtTS(nowRTP, tcCycles)
	...
	if r.srNewest != nil && nowRTPExt < r.srNewest.RTPTimestampExt {
		// 如果正在生成的报告落后，则使用编解码器的时间差和时钟频率来生成下一个报告。
		//
		// 由于以下原因，当前报告可能落后
		// - 发布者节奏
		// - 由于上述原因，发布者端的报告早于数据包时间戳。
		// 请注意，报告会将挂钟映射到捕获时间的时间戳，并在起搏器之前发生。
		// - 暂停/静音，然后恢复，某些事件组合可能导致该模块无法计算发布方的时钟速率。
		// - 当上述情况发生时，将使用可能落后的最高TS 生成电流。
		// 在极端情况下，这可能会落后于最后一个报告的时间戳
		r.logger.Infow(
...
func (r *RTPStats) DeltaOverridden(snapshotId uint32) *RTPDeltaInfo {
...
func (r *RTPStats) ToProto() *tc.RTPStats {
	r.lock.RLock()
	defer r.lock.RUnlock()
	...
	p := &tc.RTPStats{
		...
		Firs:                 r.firs,
		LastFir:              timestamppb.New(r.lastLayerLockPli),
		RttCurrent:           r.rtt,
		RttMax:               r.maxRtt,
...
func (r *RTPStats) setSnInfo(sn uint16, pktSize uint16, hdrSize uint16, payloadSize uint16, maker bool, isOutOfOrder bool) {
	writer := 0
	ooo := (sn - r.highestSN) > (1 << 15)
	if !ooo {
		writer = r.snInfoWritePtr
		r.snInfoWritePtr = (writer + 1) & SnInfoMask
	} else {
		writer = r.getSnInfoOutOfOrderPtr(sn)
		if writer < 0 {
			return
		}
	}

	snInfo := &r.snInfos[writer]
...
func (r *RTPStats) updateJitter(rtph *rtp.Header, packetTime time.Time) {
	...
	// 如果数据包无序到达。例如
	// p1f1->p1f2->p2f1
	// 在这种情况下，p2f1（分组2，帧1）仍将用于抖动计算
	// 尽管由于接收不正常，它是帧的第二个数据包。
...
func AggregateRTPStats(statsList []*tc.RTPStats) *tc.RTPStats {
	if len(statsList) == 0 {
		return nil
	}
	...
	for _, stats := range statsList {
		if startTime.IsZero() || startTime.After(stats.StartTime.AsTime()) {
			startTime = stats.EndTime.AsTime()
		}
