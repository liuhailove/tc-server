...
func (r *DependencyDescriptorReader) readExtendedFields() error {
	...
	if templateDependencyStructurePresentFlag {
		...
		if r.descriptor.AttachedStructure == nil {
			return errors.New("DependencyDescriptorReader: has templatedDependencyStructurePresentFlag but AttachedStructure is nil")
		}
...
func (r *DependencyDescriptorReader) readTemplateChains() error {
	...
	numChanins, err := r.buffer.ReadNonSymmetric(uint32(structure.NumDecodeTargets) + 1)
	if err != nil {
		return err
	}
	structure.NumChains = int(numChanins)
	...
	for _, frameTemplate := range structure.Templates {
		for chainId := 0; chainId < structure.NumChains; chainId++ {
			chanDiff, err := r.buffer.ReadBits(4)
			if err != nil {
				return err
			}
			frameTemplate.ChainDiffs = append(frameTemplate.ChainDiffs, int(chanDiff))
		}
	}
